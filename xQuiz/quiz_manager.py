...
def get_next_hint(question_id, question):
    """
    Pobiera następną podpowiedź z bazy danych lub generuje nową przy użyciu xAI.
    """
    hint = get_next_hint_db(question_id)
    if not hint:
        hint = send_to_xai(f"Podaj podpowiedź do pytania quizowego: {question}")
        if hint:
            add_hint_to_quiz(question_id, hint)
    return hint
...
def get_random_quiz_question():
    """
    Generuje losowe pytanie quizowe przy użyciu xAI.
    """
    prompt = "Wygeneruj losowe pytanie quizowe o wrestlingu z odpowiedzią i trzema podpowiedziami. Odpowiedzi mają być krótkie i proste."
    response = send_to_xai(prompt)
    if response:
        lines = response.split('\n')
        question = lines[0].replace("Pytanie: ", "").strip()
        answer = lines[1].replace("Odpowiedź: ", "").strip()
        hints = [line.replace("Podpowiedź: ", "").strip() for line in lines[2:5] if line]
        logger.info(f"Generated question data: question={question}, answer={answer}, hints={hints}")
        return {"question": question, "answer": answer, "hints": hints}
    return None

def get_random_pro_wrestling_joke():
    """
    Pobiera losowy żart związany z wrestlingiem przy użyciu xAI.
    """
    prompt = "Opowiedz śmieszny żart o profesjonalnym wrestlingu."
